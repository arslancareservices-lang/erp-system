...
# Setup paths (all files in the same folder as this script)
...
# Master CSV header (added person_id and record_id for immutability and tracking)
...
# Create history.csv if not exists
create_file_if_not_exists(HISTORY_CSV, HISTORY_HEADER)

# Create credentials.yaml if not exists (default admin/user) - FIXED WITH SECURE PASSWORDS
...
# Create sample_master.csv
SAMPLE_CONTENT = """person_id,record_id,area,pp_sz,zone,cc_uc,role,name,cnic,phone,vehicle_id,vehicle_reg_no,supervisor_id,status,updated_on,status_changed_on,last_transfer_on,remarks
P001,W001,City,PP-110,Zone-01,CC-001,Sanitary Worker,Bilal Ahmed,3520212345671,03001234567,,,W010,active,2025-11-01 00:00:00,,,
